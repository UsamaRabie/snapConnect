...
export function TaggedPostsGrid({ userId }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const result = await postApi.getTagged(userId, 1);
  ...
  if (posts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-xl border border-dark-600 bg-dark-800 py-16">
        <Tag size={32} className="text-dark-500 mb-2" />
        <p className="text-dark-400">No tagged posts yet</p>
      </div>
    );
  }
